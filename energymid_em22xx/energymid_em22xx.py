...
class EM22xxModbus:
   ...
   def read_input_register(self, register_address, datatype, count=1) -> list:
      ...
      try:
         result = self._client.read_input_registers(
            register_address, count=length, device_id=self._device_unit_id
            )
   ...
   def read_holding_register(self, register_address, datatype, count=1) -> list:
      ...
      # print(f'length : {length}')
      try:
         result = self._client.read_holding_registers(register_address, count=length, device_id=self._device_unit_id)
         # print(result, type(result))
         if isinstance(result, ExceptionResponse):
